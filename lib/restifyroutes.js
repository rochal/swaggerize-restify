    ...
    routes.forEach(function (route) {
        ...
        route.validators.forEach(function (validator) {
            ...
            before.push(function validateInput(req, res, next) {
                var value, isPath, isQuery, isBody, isFormData;
                ...
                validate(value, function (error, newvalue) {
                    ...
                    // only replace body with validated version if it's available as req.body
                    // (not req.params, where path params can be kept, too)
                    if (isBody && req.body) {
                        req.body = newvalue;
                    }

                    if (isFormData && req.body) {
                        req.body[parameter.name] = newvalue;
                    }
